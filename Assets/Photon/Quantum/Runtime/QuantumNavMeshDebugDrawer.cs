namespace Quantum {
  ...
  public class QuantumNavMeshDebugDrawer : QuantumMonoBehaviour {
    public Quantum.AssetRef<Quantum.BinaryData> BinaryAsset;
    ...
    private Quantum.NavMesh _navmesh;
    ...
    void OnDrawGizmosSelected() {
      ...
      var asset = (Quantum.BinaryData)QuantumUnityDB.GetGlobalAsset(BinaryAsset);
      if (asset == null) {
        return;
      }

      var stream = new ByteStream(asset.Data);
      _navmesh = AssetObject.Create<NavMesh>();
      _navmesh.Serialize(stream, false);
      _navmesh.Name = asset.Identifier.Path;

      QuantumNavMesh.CreateAndDrawGizmoMesh(_navmesh, NavMeshRegionMask.Default, GlobalGizmosSettings);
      ...
      if (DrawLinks) {
        for (int i = 0; i < _navmesh.Links.Length; i++) {
          ...
          GizmoUtils.DrawGizmoVector(
            ...
            GizmoUtils.DefaultArrowHeadLength * GlobalGizmosSettings.GizmoIconScale.AsFloat);
        }
      }
        ...
        for (int i = 0; i < _navmesh.Vertices.Length; i++) {
          ...
          if (DrawVertexNormals) {
            ...
            GizmoUtils.DrawGizmoVector(p,
              p + normal * GlobalGizmosSettings.GizmoIconScale.AsFloat * 0.33f,
              GlobalGizmosSettings.GizmoIconScale.AsFloat * 0.33f * GizmoUtils.DefaultArrowHeadLength);
          }
        }
      }
        ...
        for (int i = 0; i < _navmesh.Borders.Length; i++) {
          ...
          if (DrawBorderNormals) {
            ...
            GizmoUtils.DrawGizmoVector(middle,
              middle + _navmesh.Borders[i].Normal.ToUnityVector3(true) * GlobalGizmosSettings.GizmoIconScale.AsFloat * 0.33f,
              GlobalGizmosSettings.GizmoIconScale.AsFloat * 0.33f * GizmoUtils.DefaultArrowHeadLength);
          }
        }
      }
    ...
    static void DrawTriangle(int i, Quantum.NavMesh navmesh) {
    ...
    static void DrawTriangleMesh(int i, Quantum.NavMesh navmesh, Color color) {
      var t       = navmesh.Triangles[i];
      var vertex0 = navmesh.Vertices[t.Vertex0].Point.ToUnityVector3(true);
      var vertex1 = navmesh.Vertices[t.Vertex1].Point.ToUnityVector3(true);
      var vertex2 = navmesh.Vertices[t.Vertex2].Point.ToUnityVector3(true);
    ...
    [CustomEditor(typeof(QuantumNavMeshDebugDrawer))]
    private class MapNavMeshDebugDrawerEditor : UnityEditor.Editor {
      public override void OnInspectorGUI() {
        base.OnInspectorGUI();

        var data = (QuantumNavMeshDebugDrawer)target;
