...
public class MarketplaceListing : MonoBehaviour
{
    ...
    public async Task Init(string marketplaceContract, MarketplaceModel.Item item, MarketplaceModel.MarketplaceItemMetaData metaData)
    {
        ...
        nftPrice.text = ethValue.ToString("0.##################") + " " + Web3Unity.Web3.ChainConfig.Symbol.ToUpper();
    ...
    public async void PurchaseNFT()
    {
        ...
        var previousChain = Web3Unity.Web3.Chains.Current.ChainId;
        try
        {
            await Web3Unity.Web3.Chains.SwitchChain("1993");
            var nft = await EvmMarketplace.PurchaseNft(_marketplaceContract, _item.id, _item.price);
            Debug.LogError(nft.TransactionHash);
            IMainThreadRunner mainThreadRunner =
                (IMainThreadRunner)Web3Unity.Web3.ServiceProvider.GetService(typeof(IMainThreadRunner));
            mainThreadRunner.Enqueue(() => Destroy(gameObject));
        }
        catch (Exception e)
        {
            
            Debug.LogError("Excpetion thworn" + e.Message);
        }
        finally
        {
            button.interactable = true;
            await Web3Unity.Web3.Chains.SwitchChain(previousChain);
        }
    }
}
